...
最后一种方式可以让我们指定切片的初始容量。当我们大概知道需要多少元素时很有用。。即使你之道元素的个数，`append`也能被使用，这主要取决于个人喜好：
...
从上面例子中，我们可以看出`[X:]`表示`X`到结尾的一种缩写。而`[:X]`是开始到`X`的一种缩写。不像其他语言，go不支持负值索引。如果我们想要除了最后一个元素之外的切片所有元素，我们可以：
